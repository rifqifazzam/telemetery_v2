...
class FloatingActionButton:
    """Draggable floating action button with control buttons."""
    ...
    def create_window(self):
        ...
        # Status indicator
        indicator_frame = tk.Frame(header_frame, bg=Config.COLORS['bg_tertiary'])
        ...
        # Bind dragging events to header
    ...
    def destroy(self):
        """Destroy the floating action button window."""
        if self.window:
            self.window.destroy()
            self.window = None
    ...
    def update_button_states(self, state: str):
        ...
        if state == 'running':
            ...
            self.is_paused = True
            self.update_indicator('warning')
        elif state == 'stopped':
            self.pause_btn.config(state=tk.DISABLED)
            self.stop_btn.config(state=tk.DISABLED)
            self.pause_btn.config(text="Pause")
            self.is_paused = False
            self.update_indicator('danger')
